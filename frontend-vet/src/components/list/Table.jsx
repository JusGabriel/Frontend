...
import storeAuth from "../../context/storeAuth"; // Importamos el store
...
const Table = ({ entidad }) => {
  ...
  const { token, rol } = storeAuth(); // Obtenemos token y rol del administrador

  const listarRegistros = async () => {
    const url = `${import.meta.env.VITE_BACKEND_URL}/api/${entidad}/todos`;
  ...
  const handleDelete = async (id) => {
    ...
    const url = `${import.meta.env.VITE_BACKEND_URL}/api/clientes/eliminar/${id}`;
    ...
    if (response?.msg === "Cliente eliminado correctamente") {
      setRegistros(registros.filter(cliente => cliente._id !== id));
    } else {
